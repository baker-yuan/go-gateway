...
// PluginWorkerConfig https://help.apinto.com/docs/apinto/plugins/
...
type PluginConfig struct {
	Name       string                 `json:"name" yaml:"name" `              // 插件名/插件别名, 在列表中唯一
	ID         string                 `json:"id" yaml:"id"`                   // 插件id，{group}:{project}:{name}
	Status     string                 `json:"status" yaml:"status"`           // 插件状态
	Config     interface{}            `json:"config" yaml:"config"`           // 插件的全局配置，当status=global时生效，内容由对应插件决定
