...
import (
	...
	"github.com/baker-yuan/go-gateway/go-gateway-admin/internal/data/ent/migrate"
...
func (c *Client) Intercept(interceptors ...Interceptor) {
	c.HttpRule.Intercept(interceptors...)
}

// Mutate implements the ent.Mutator interface.
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	switch m := m.(type) {
	case *HttpRuleMutation:
		return c.HttpRule.mutate(ctx, m)
	default:
...
// A call to `Intercept(f, g, h)` equals to `httprule.Intercept(f(g(h())))`.
func (c *HttpRuleClient) Intercept(interceptors ...Interceptor) {
	c.inters.HttpRule = append(c.inters.HttpRule, interceptors...)
}

// Create returns a builder for creating a HttpRule entity.
...
// CreateBulk returns a builder for creating a bulk of HttpRule entities.
...
func (c *HttpRuleClient) UpdateOneID(id uint32) *HttpRuleUpdateOne {
...
func (c *HttpRuleClient) DeleteOneID(id uint32) *HttpRuleDeleteOne {
...
func (c *HttpRuleClient) Query() *HttpRuleQuery {
	return &HttpRuleQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeHttpRule},
		inters: c.Interceptors(),
	}
}

// Get returns a HttpRule entity by its id.
func (c *HttpRuleClient) Get(ctx context.Context, id uint32) (*HttpRule, error) {
	return c.Query().Where(httprule.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *HttpRuleClient) GetX(ctx context.Context, id uint32) *HttpRule {
...
func (c *HttpRuleClient) Interceptors() []Interceptor {
	return c.inters.HttpRule
}

func (c *HttpRuleClient) mutate(ctx context.Context, m *HttpRuleMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&HttpRuleCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&HttpRuleUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&HttpRuleUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&HttpRuleDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown HttpRule mutation op: %q", m.Op())
	}
}
